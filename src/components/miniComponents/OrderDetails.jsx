...
import { orderSchema } from '../schemas/formValidate';
import './OrderDetails.css'

function OrderDetails({name, id, category, price, planType, cancel}) {
    ...
    const onSubmit = async(data, e) => {
        ...
        try {
            emailJs.send('service_ypi7l5l', 'template_lplb987', order, 'user_LhB6BmKHXG6o83l1skINd')
            .then((result) => {
                    toast.success('Thank you for your order. Our team will contact you soon to arrange for payment and delivery of your order.', {
                        position : 'top-right',
                        autoClose : 5000,
                        theme : 'light'
                    })
    ...
    return (
        <div className="order-details-cont">
            <div className="order-details" ref={formRef}>
                <h3>Please enter details to confirm order of this item</h3>
                <p><span>Plan/Product Name:</span> <span>{name?.slice(0, 38)}...</span></p>
                <p><span>ID:</span> <span>{id}</span></p>
                <p><span>Category:</span> <span>{category}</span></p>
                <p><span>Price:</span> <span>{price}</span></p>
                <p><span>Plan/Product Type:</span> <span>{planType}</span></p>

                <div className="contact-details">
                    <form ref={formRef} className='order-details-form' onSubmit={handleSubmit(onSubmit)} id='form'>
                        {
                            errorMessage && <p className="error">{errorMessage}</p>
                        }
                        <div className="order-details-form-container">
                            ...
                            <div className="btnSub-cont">
                                <input type="button" value="Cancel" className='btnCan' onClick={cancel} />
